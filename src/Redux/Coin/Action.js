...
export const fetchCoins = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(`${API_BASE_URL}/coins/coins`);
    dispatch({ type: FETCH_COINS_SUCCESS, payload: response.data });
    console.log("coinss=", response.data);
  } catch (error) {
...
export const getTopGainers = () => async (dispatch) => {
  dispatch({ type: FETCH_TOP_GAINER_COINS_REQUEST });
  try {
    const response = await axios.get(`${API_BASE_URL}/coins/top-gainers`);
    dispatch({ type: FETCH_TOP_GAINER_COINS_SUCCESS, payload: response.data });
    console.log("top gainer", response.data);
    console.log("type", typeof response.data);
  } catch (error) {
    console.log("error", error);
    dispatch({ type: FETCH_TOP_GAINER_COINS_FAILURE, payload: error.message });
  }
};
export const getTopLosers = () => async (dispatch) => {
  dispatch({ type: FETCH_TOP_LOSER_COINS_REQUEST });
  try {
    const response = await axios.get(`${API_BASE_URL}/coins/top-losers`);
    dispatch({ type: FETCH_TOP_LOSER_COINS_SUCCESS, payload: response.data });
    console.log("top loser", response.data);
  } catch (error) {
    console.log("error", error);
    dispatch({ type: FETCH_TOP_LOSER_COINS_FAILURE, payload: error.message });
  }
};
